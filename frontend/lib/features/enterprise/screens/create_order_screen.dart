...
class _CreateOrderScreenState extends State<CreateOrderScreen> {
  ...
  Future<void> _loadProducts() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _products = results[0].where((p) => p['is_active'] == true).toList();
  ...
  Future<void> _pickAddressFromMap() async {
    final result = await Navigator.push<PickedLocation>(
      context,
      MaterialPageRoute(builder: (_) => const MapPickerPage(needAddress: true)),
    );
  ...
  String _friendlyError(Object error) {
    if (error is DioException) {
      ...
      if (error.type == DioExceptionType.connectionTimeout ||
        ...
        return 'Сервер жооп бербей жатат. Интернетти текшерип кайра аракет кылыңыз.';
      }
      return error.message ?? 'Заказ түзүүдө ката кетти';
    }
  ...
  Future<void> _submit() async {
    ...
    try {
      final data = {
        ...
        if (_deliveryLocation != null) 'to_lat': _deliveryLocation!.lat,
        if (_deliveryLocation != null) 'to_lng': _deliveryLocation!.lon,
                      ...
                      _section('Товарлар', [
                        if (_products.isEmpty)
                          const Padding(
                            ...
                            child: Center(
                              child: Text(
                                'Товар жок. Меню бөлүмүнөн кошуңуз.',
