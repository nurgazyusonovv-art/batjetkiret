...
class PaymentsScreen extends StatefulWidget {
  const PaymentsScreen({super.key, this.focusOrderId});
...
class _PaymentsScreenState extends State<PaymentsScreen>
  ...
  void initState() {
    super.initState();
    _focusedOrderId = widget.focusOrderId;
  ...
  void didUpdateWidget(covariant PaymentsScreen oldWidget) {
    ...
    if (widget.focusOrderId != null &&
        widget.focusOrderId != oldWidget.focusOrderId) {
      _focusedOrderId = widget.focusOrderId;
      _load(silent: true);
    }
  }
  ...
  void _selectFocusedTab() {
    final id = _focusedOrderId;
    if (id == null) return;
    final inPending = _pending.any((p) => p['order_id'] == id);
    final inConfirmed = _confirmed.any((p) => p['order_id'] == id);
    if (inPending) {
      _tabs.index = 0;
    } else if (inConfirmed) {
      _tabs.index = 1;
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            Tab(
              text:
                  'Күтүүдө${_pending.isNotEmpty ? " (${_pending.length})" : ""}',
            ),
          ...
          : TabBarView(
              ...
              children: [
                _PaymentList(
                  payments: _pending,
                  isPending: true,
                  focusOrderId: _focusedOrderId,
                  onConfirm: _confirm,
                  onReject: _reject,
                  onRefresh: _load,
                  emptyText: _focusedOrderId == null
                      ? 'Күтүп жаткан төлөм жок'
                      : 'Бул заказга күтүп жаткан төлөм жок',
                ),
                _PaymentList(
                  payments: _confirmed,
                  isPending: false,
                  ...
                  onRefresh: _load,
                  emptyText: _focusedOrderId == null
                      ? 'Тастыкталган төлөм жок'
                      : 'Бул заказга тастыкталган төлөм жок',
                ),
              ],
            ),
    );
...
class _PaymentList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final visiblePayments = _orderedPayments();
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) => _PaymentCard(
          ...
          isPending: isPending,
          isFocused: visiblePayments[i]['order_id'] == focusOrderId,
          onConfirm: onConfirm,
          onReject: onReject,
        ),
      ),
  ...
  List<dynamic> _orderedPayments() {
    final id = focusOrderId;
    if (id == null) return payments;
    final copy = [...payments];
    copy.sort((a, b) {
      final aMatch = a['order_id'] == id;
      final bMatch = b['order_id'] == id;
            ...
            child: Column(
              ...
              children: [
                ...
                if (screenshotUrl != null && screenshotUrl.isNotEmpty) ...[
                  ...
                  const SizedBox(height: 4),
                  const Text(
                    'Скриншотту чоңойтуу үчүн басыңыз',
