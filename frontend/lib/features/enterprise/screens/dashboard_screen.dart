      ...
      if (mounted) {
        setState(() {
          ...
          _error =
              ...
              ? 'Сессиянын мөөнөтү бүттү. Профилден чыгып, кайра кириңиз.'
...
class _OpenStatusCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bgColor = isOpen ? const Color(0xFFDCFCE7) : const Color(0xFFFEF2F2);
                ...
                children: [
                  ...
                  _card([
                    Row(
                      children: [
                        _sectionTitle(
                          items.isNotEmpty ? '🛍 Товарлар' : '📋 Заказ мазмуну',
                        ),
                        const Spacer(),
                        if (_loading)
                          const SizedBox(
...
class _ActiveOrderTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final total =
        (order['items_total'] as num?)?.toStringAsFixed(0) ??
